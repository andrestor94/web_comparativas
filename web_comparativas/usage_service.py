    ...
    *,
    ...
    try:
        ...
        for ev in events:
            ...
            sec = (ev.section or "").strip() or "(sin sección)"
            sec_st = section_stats.setdefault(
                sec, {"section": sec, "events": 0, "user_ids": set()}
            )
            sec_st["events"] += 1
        ...
        # b) Analistas vs Supervisores (agrupado)
        role_chart = []
        for r_key, rs in role_stats.items():
            label = "Analistas" if r_key in ("analista", "analyst") else (
                "Supervisores" if r_key == "supervisor" else "Otros"
            )
            role_chart.append(
                {
                    "role_key": r_key,
                    "role_label": label,
                    "events": int(rs["events"]),
                    "uploads": int(rs["uploads"]),
                    "active_minutes": round(float(rs["active_minutes"]), 1),
                }
            )
        ...
        return {
            ...
            "charts": {
                ...
                "analysts_vs_supervisors": role_chart,
