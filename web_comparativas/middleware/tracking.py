...
from web_comparativas import tracking_taxonomy  # FUENTE ÚNICA de detección de sección

logger = logging.getLogger("wc.tracking")


# ─── Detección de sección ────────────────────────────────────────────────────
# Una sola definición, delega 100% en la taxonomía única (tracking_taxonomy):
# misma fuente que usa el labeler (_map_section_name) y el front. Antes había
# DOS definiciones de _detect_section (la segunda pisaba a la primera) más un
# fallback heurístico propio; todo eso se reemplazó por la fuente canónica.

def _detect_section(path: str) -> str:
    return tracking_taxonomy.detect_key(path)
...
_EXCLUDED_PREFIXES = (
    ...
    "/healthz",
    "/ping",
    "/api/section-taxonomy.js",  # asset generado (taxonomía); no es navegación
