...
# Adapter "La Pampa" (UI/pipeline)
# Basado en tu script ORIGINAL (el que funciona perfecto) pero adaptado a:
#   normalize_la_pampa(input_path, metadata, out_dir)
#
# Acepta:
#   - .zip con PDFs (recomendado)
#   - .pdf suelto (busca el otro PDF en la misma carpeta)
#   - directorio con PDFs
#
# Output:
#   - return {"df": std_df, "summary": summary}
#   - además escribe en out_dir:
#       * la_pampa_master.xlsx
#       * la_pampa_estandar.xlsx
#       * normalized.xlsx   (este es el que suele consumir la interfaz)
#
# FIX CRITICO:
#   - Si un PDF se detecta como comparativa y pliego a la vez,
#     priorizamos COMPARATIVA y lo excluimos de PLIEGOS (evita merges vacíos).
...
def _assign_band(x: float, cuts: List[float]) -> int:
    ...
    if x < cuts[3]: return 3  # UNIDAD (y comienzo desc)
    return 4                  # DESCRIPCION
...
def leer_pliego_pdf(path_pdf: str) -> pd.DataFrame:
    """
    Tu parser original (sin cambios de lógica):
      - columnas por tríos numéricos
      - UNIDAD banda 3 + prefijo de descripción
...
def leer_comparativa_pdf(path_pdf: str) -> pd.DataFrame:
    """
    Tu parser original robusto (sin cambios de lógica):
      - Columnas SOLO si token incluye 'Alt'
      - Proveedor a la izquierda
      - PU alineado por columna
      - ignora TOTAL/PROMEDIO/PROVEEDOR
    """
    try:
        import pdfplumber
    except Exception:
...
def _collect_pdfs(input_path: Path, out_dir: Path) -> Tuple[List[Path], Optional[Path]]:
    ...
    raise ValueError("La Pampa: se espera .zip, .pdf o directorio con PDFs.")
...
def normalize_la_pampa(input_path: Path, metadata: Dict[str, Any], out_dir: Path) -> Dict[str, Any]:
    """
    Adapter para la interfaz/pipeline.
    - Acepta ZIP (recomendado) con PDFs, o PDF suelto (busca pareja en el mismo dir), o directorio.
    - Construye Master + Standard, y SIEMPRE escribe normalized.xlsx (que tu backend espera).
    - Devuelve {"df": std_df, "summary": {...}}.
    """
    out_dir = Path(out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)

    pdf_paths, _extract_dir = _collect_pdfs(Path(input_path), out_dir)
    if not pdf_paths:
    ...
    pliegos_all  = [str(p) for p in pdf_paths if _looks_like_pliego_pdf(str(p))]

    # FIX CRITICO: priorizar comparativa si un PDF cae en ambos buckets
    compar_set = set(comparativas)
    pliegos = [p for p in pliegos_all if p not in compar_set]
    ...
    # Pairing: caso UI típico (1+1) => directo
    pairs: List[Tuple[str, str, str, str]] = []  # (comp, pliego, tag, expediente)

    if len(comparativas) == 1 and len(pliegos) == 1:
        comp = comparativas[0]
        plg  = pliegos[0]
        expediente = _extract_expediente_from_pdf(plg) or _extract_expediente_from_pdf(comp)
        tag = _find_tag_in_filename(comp) or _find_tag_in_filename(plg) or _tag_expediente(expediente)
        pairs.append((comp, plg, tag, expediente))
    else:
        for comp in sorted(comparativas):
            ...
            pairs.append((comp, pliego_file, tag_name, expediente))
    ...
    for comp, pliego_file, tag_name, expediente in pairs:
        df_pl   = leer_pliego_pdf(pliego_file)
        df_comp = leer_comparativa_pdf(comp)

        # robustez merge: ITEM como int
        for d in (df_pl, df_comp):
            if "ITEM" in d.columns:
        ...
        df = (df[cols]
              .sort_values(["Tag", "ITEM", "Alt", "Posicion", "PROVEEDOR"], na_position="last")
    ...
    # Guardar outputs en out_dir
    master_path = out_dir / "la_pampa_master.xlsx"
    standar_path = out_dir / "la_pampa_estandar.xlsx"
    ...
    try:
        ...
        with pd.ExcelWriter(standar_path, engine="openpyxl") as xw:
            std_df.to_excel(xw, sheet_name="Estandar", index=False)

        # ESTE ES EL QUE ESPERA TU INTERFAZ
        with pd.ExcelWriter(normalized_path, engine="openpyxl") as xw:
    ...
    summary = {
        ...
        "comparativas": int(len(comparativas)),
        "pliegos": int(len(pliegos)),
        "pliegos_raw": int(len(pliegos_all)),  # útil para debug
        "pairs_processed": int(len(master_rows)),
        "rows_master": int(len(master_df)),
        "rows_standard": int(len(std_df)),
        "out_dir": str(out_dir),
        "outputs": {
            "master": str(master_path),
            "estandar": str(standar_path),
