...
(function () {
  ...
  // Rango inicial de fechas (Lógica: hoy + 4 días, saltando findes)
  let initFrom = null;
  let initTo = null;
  ...
  if (uiFrom) {
    ...
    // Cálculo solicitado: "A partir de 3 días desde hoy".
    // Ejemplo usuario: 9/12 -> 13/12 (9+4). Si cae finde -> lunes.
    const today = new Date();
    // Offset de 4 días (para cumplir "posterior a 3 días")
    let target = new Date(today);
    target.setDate(today.getDate() + 4);

    // Ajuste si cae sábado o domingo
    const day = target.getDay();
    if (day === 6) { // Sábado -> Lunes (+2)
      target.setDate(target.getDate() + 2);
    } else if (day === 0) { // Domingo -> Lunes (+1)
      target.setDate(target.getDate() + 1);
    }

    // Normalizar a inicio del día (00:00:00)
    target.setHours(0, 0, 0, 0);

    initFrom = target;
    // Hasta el final (null o DOMAIN_MAX si queremos acotar al dataset)
    // Dejamos null para que el filtro sea logicamente ">= initFrom"
    // Pero para el slider visual, necesitamos un tope.
    initTo = DOMAIN_MAX ? new Date(DOMAIN_MAX) : null;
  }
