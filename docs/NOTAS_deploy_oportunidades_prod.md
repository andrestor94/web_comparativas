...
| `CRM_CA_BUNDLE` | — | Ruta a un archivo `.pem` local con el intermedio (uso en desarrollo, generado con `scripts/crm_ca_bundle.py`). |
| `CRM_CA_PEM` | — | **Para Render** (ago-2026): mismo intermedio que `CRM_CA_BUNDLE` pero como CONTENIDO de env var, no ruta de archivo — Render no tiene el `.pem` local (está gitignoreado). Al arrancar, si está seteada, se escribe a un archivo temporal (certifi + este intermedio) y ahí apunta la verificación. Sin esto y sin `CRM_CA_BUNDLE`, se usa certifi normal (sin el intermedio agregado) y el CRM puede fallar con "unable to get local issuer certificate" si no manda la cadena completa. |
...
3. **Datos:** `oportunidades_summary` se calcula LOCAL (`scripts/rebuild_oportunidades.py --run-id <run local espejo del activo de prod>`) y viaja como DATO con `scripts/push_oportunidades_data.py --url ... --token ...` (patrón `push_match_data`: endpoint admin `POST /api/mercado-privado/oportunidades/admin/apply-data-chunk` protegido con el mismo `X-Import-Token`, reset run-scoped en el primer lote, reanudable — construido ago-2026). Son ~150 filas: un solo lote alcanza, pero respeta el patrón. Estado verificable con `GET /api/mercado-privado/oportunidades/admin/estado`. NUNCA correr el rebuild server-side en Render (lee todos los records del run).
