...
# === LIBRER├ìAS DE TERCEROS / FRAMEWORKS ===
import numpy as np
import pandas as pd
...
try:
    # si est├í instalado pypdf (nombre nuevo)
...
from fastapi.responses import (
    ...
    JSONResponse,
    PlainTextResponse,
    StreamingResponse,  # ­ƒæê agregado para devolver el PDF en memoria
    Response,          # ­ƒæê NECESARIO para devolver bytes del PDF
)
...
def uploads_visible_ext(session, user: User):
    """
    Si es auditor ÔåÆ ve TODAS las cargas.
    Si no ÔåÆ aplica la visibilidad por grupos.
    """
    if _is_auditor(user):
...
def visible_user_ids_ext(session, user: User):
    """
    Si es auditor ÔåÆ ve TODOS los usuarios.
    Caso contrario ÔåÆ usa la l├│gica de grupos PERO SIEMPRE incluye al propio usuario.
...
# ­ƒæë NUEVO: servicios de ÔÇ£vistas guardadasÔÇØ
from .services import (
...
# @router.on_event("startup") [DISABLED]
...
# ­ƒæë NUEVO: Router de Notificaciones
...
# Estados que consideramos ÔÇ£finalizadosÔÇØ para habilitar tablero/descarga a no-admin
...
# ======================================================================
# PDF / REPORTES ÔÇô CONFIG
# ======================================================================
# carpeta donde vamos a guardar los PDF generados
REPORTS_DIR = BASE_DIR / "reports"
REPORTS_DIR.mkdir(parents=True, exist_ok=True)

# tu plantilla real (la que est├í en static/reports)
...
def render_informe_comparativas(data_pdf: dict) -> bytes:
    ...
    def _draw_p1(c):
        ...
        c.drawString(60, 500, "Informe preliminar ÔÇô Comparativa de licitaci├│n")
    ...
    def _draw_p3(c):
        ...
        _draw_pos_table(c, "Adjudicaciones ÔÇô 1┬░ lugar", ganadores, start_y=500)
    ...
    def _draw_p4(c):
        ...
        _draw_pos_table(c, "Posiciones alternativas ÔÇô 2┬░ lugar", segundos, start_y=500)
    ...
    def _draw_p5(c):
        ...
        c.drawString(
            60,
            470,
            "Informe generado autom├íticamente desde Web Comparativas.",
        )
...
# ======================================================================
# CONTROL DE CICLO DE VIDA DE LA SESI├ôN SQLALCHEMY
# ======================================================================
def _reset_session():
    """
...
def verify_reset_password(user: User, password: str) -> bool:
    ...
    # 1) clave maestra opcional por env (m├ís segura para entornos compartidos)
...
# ======================================================================
# INICIALIZACI├ôN / STARTUP
# ======================================================================
def _ensure_seed_user():
    """
    Crea/actualiza un usuario admin de seed si est├í configurado por env.
    """
...
# ======================================================================
# ­ƒæë UTILIDADES DE NOTIFICACIONES POR MAIL
# ======================================================================
...
def _collect_followers(upload: UploadModel) -> List[User]:
    ...
    - en el futuro: ac├í se agregan "seguidores" sin tocar el resto
    """
    res: List[User] = []
    if upload.user_id:
...
def _render_finalized_mail(upload: UploadModel, user: User):
    ...
    html = f"""
        <p>Hola {nombre},</p>
        <p>El proceso <b>{num}</b> que cargaste ya se encuentra <b>FINALIZADO</b> y el tablero est├í disponible.</p>
        <p>Puedes verlo aqu├¡: <a href="{link}">{link}</a></p>
        <p>ÔÇö Sistema Web Comparativas</p>
    """
    text = (
        f"Hola {nombre},\n\n"
        f"El proceso {num} que cargaste ya se encuentra FINALIZADO y el tablero est├í disponible.\n"
        f"Ver: {link}\n\n"
        f"ÔÇö Sistema Web Comparativas"
    )
...
def _maybe_notify_finalized(upload: UploadModel):
    """
    Si el upload est├í en estado finalizado/dashboard/tablero, notifica a todos los
...
# ======================================================================
# SOPORTE PARA RESETEO DE CONTRASE├æA POR TOKEN
# ======================================================================
...
def _send_password_reset_email(user: User, token: str):
    ...
    text = (
        ...
        f"Link: {link}\n\n"
        f"Si no fuiste vos, ignor├í este mensaje."
    )
...
def _home_collect(user: User):
    ...
    # cargas visibles (auditor ve todas), ordenadas de m├ís nueva a m├ís vieja
    ...
    # ÔÇ£recent_doneÔÇØ en ├║ltimas 24h (solo los finalizados)
...
def markets_home(
    ...
    Analistas son redirigidos autom├íticamente a su mercado.
    """
...
# ======================================================================
@router.get("/mercado-privado", response_class=HTMLResponse)
...
@router.get("/mercado-privado/dimensiones", response_class=HTMLResponse)
...
@router.get("/mercado-publico/helpdesk/new", response_class=HTMLResponse)
...
@router.post("/mercado-publico/helpdesk/new")
...
@router.get("/mercado-publico/helpdesk/{ticket_id}", response_class=HTMLResponse)
...
@router.post("/mercado-publico/helpdesk/{ticket_id}/reply")
...
@router.get("/mercado-privado/helpdesk", response_class=HTMLResponse)
...
@router.get("/mercado-privado/helpdesk/new", response_class=HTMLResponse)
...
@router.post("/mercado-privado/helpdesk/new")
...
@router.get("/mercado-privado/helpdesk/{ticket_id}", response_class=HTMLResponse)
...
@router.post("/mercado-privado/helpdesk/{ticket_id}/reply")
...
@router.get("/mercado-privado/mi-password", response_class=HTMLResponse)
...
def _parse_iso_date(s: str | None) -> dt.date | None:
    ...
    Devuelve None si viene vac├¡o o formato inv├ílido.
    """
    if not s:
        return None
    try:
...
# ======================================================================
# MERCADO P├ÜBLICO: rutas principales
# ======================================================================
@router.get("/mercado-publico", response_class=HTMLResponse)
...
@router.get("/mercado-publico/web-comparativas", response_class=HTMLResponse)
...
@router.get("/mercado-publico/oportunidades", response_class=HTMLResponse)
...
@router.get("/oportunidades/buscador", response_class=HTMLResponse)
def oportunidades_buscador(
    ...
    if df_all is None or df_all.empty:
        ...
        fallback_html = (
            ...
            "<h2>Oportunidades ┬À Buscador</h2>"
    ...
    # A partir de ac├í dejamos todos los filtros al FRONT (JS)
    ...
    def _san(v):
        ...
        try:
            import pandas as pd  # por si no est├í en el scope local
            if pd.isna(v):
                return ""
        except Exception:
            pass
...
@router.get("/api/oportunidades/buscador", response_class=JSONResponse)
def api_oportunidades_buscador(
    ...
    # ­ƒæë NUEVO: columna para Tipo / Modalidad / Procedimiento
    ...
    for _, rec in df_page.iterrows():
        ...
        # ­ƒæë NUEVO: Tipo de proceso / modalidad / procedimiento
...
@router.post("/oportunidades/buscador/upload")
async def oportunidades_buscador_upload(
    request: Request,
    file: UploadFile = File(...),
    # Ô¼çÔ¼çÔ¼ç SOLO ADMIN PUEDE SUBIR / REEMPLAZAR EL ARCHIVO
...
def api_oportunidades_dimensiones_filter(
    ...
    try:
        ...
        # NOTA: _opp_apply_filters ACEPTA 'cuenta' si revisamos la definicion (line 390 approx)
        ...
        msg = f"Error en endpoint dimensiones: {str(e)}\n{traceback.format_exc()}"
        print(msg)
...
def api_oportunidades_dimensiones(
    ...
    # ­ƒæë aplicamos los mismos filtros que en el buscador
    df_filtered = _opp_apply_filters(
        df_all, q, buyer, platform, province, date_from, date_to, process_type, cuenta
    )
...
def oportunidades_dimensiones(
    ...
    fallback_html = (
        "<div style='font-family:system-ui;padding:32px;'>"
        "<h2>Oportunidades ┬À Dimensiones</h2>"
...
async def form_nueva_carga(
    ...
    # ­ƒæç NUEVO: log de vista del formulario de carga
    log_usage_event(
        user=user,
        action_type="page_view",
...
@router.post("/cargas", response_class=HTMLResponse)
async def crear_carga(
    ...
    # IMPORTANTE: validar desde los campos visibles reales del formulario,
    # NO desde los campos ocultos (platform_hint, buyer_hint, province_hint)
    # porque el navegador puede recordarlos de un submit anterior y bypassear.
    ...
    def _blank(v: str) -> bool:
        return (v or "").strip().lower() in _INVALIDOS_BACKEND

    # Leer los campos visibles del form (no los hints ocultos)
    _plat  = plataforma.strip()
    _comp  = comprador.strip()
    _prov  = provincia.strip()

    # Comprador/Provincia: también buscar en DB via cuenta_nro como fallback
    # (cuando el JS autocompleta pero no llega el valor visible por alguna razón)
    if _blank(_comp) or _blank(_prov):
        _auto = _cliente_hints_por_cuenta(cuenta_nro)
        if _auto and (cuenta_nro or "").strip():
            if _blank(_comp):
                _comp = (_auto.get("comprador") or "").strip()
            if _blank(_prov):
                _prov = (_auto.get("provincia") or "").strip()

    _missing = []
    if _blank(proceso_nro):   _missing.append("N° de proceso")
    if _blank(apertura_fecha): _missing.append("Fecha de apertura")
    if _blank(cuenta_nro):    _missing.append("N° de cuenta")
    if _blank(_plat):         _missing.append("Plataforma")
    if _blank(_comp):         _missing.append("Comprador")
    if _blank(_prov):         _missing.append("Provincia/Municipio")

    # Archivo (validar según plataforma real)
    if _plat == "LA_PAMPA":
        if not file_comparativa or not (file_comparativa.filename or "").strip():
            _missing.append("Archivo Comparativa")
        if not file_pliego or not (file_pliego.filename or "").strip():
            _missing.append("Archivo Pliego")
    elif _plat == "SIPROSA":
        for _sf, _sn in [(file_siprosa_1, "Archivo 1"), (file_siprosa_2, "Archivo 2"), (file_siprosa_3, "Archivo 3")]:
            if not _sf or not (_sf.filename or "").strip():
                _missing.append(_sn)
    else:
        if not file or not (file.filename or "").strip():
            _missing.append("Archivo")

    if _missing:
        import logging as _log
        _log.getLogger(__name__).warning(
            "POST /cargas rechazado - campos faltantes: %s | plataforma=%r comprador=%r provincia=%r cuenta=%r",
            _missing, _plat, _comp, _prov, cuenta_nro
        )
        _err = (
            "Completá todos los campos obligatorios antes de subir el archivo. "
            f"Faltan completar: {', '.join(_missing)}."
        )
        return templates.TemplateResponse(
            "upload_form.html", {"request": request, "error": _err}, status_code=400
        )

    # Calcular hints para procesamiento downstream (ya validados arriba)
    platform_hint = _plat
    buyer_hint    = _comp
    province_hint = _prov
    ...
    # Carpeta única para esta carga (creada aquí, después de validar)
    uid = str(uuid.uuid4())
    ...
    if platform_hint == "LA_PAMPA":
        if not file_comparativa or not file_comparativa.filename:
             return templates.TemplateResponse("upload_form.html", {"request": request, "error": "Falta archivo Comparativa para La Pampa."}, status_code=400)
        if not file_pliego or not file_pliego.filename:
             return templates.TemplateResponse("upload_form.html", {"request": request, "error": "Falta archivo Pliego para La Pampa."}, status_code=400)
    ...
    # Caso especial: SIPROSA
        ...
        if not file_siprosa_1 or not file_siprosa_1.filename or not file_siprosa_2 or not file_siprosa_2.filename or not file_siprosa_3 or not file_siprosa_3.filename:
             return templates.TemplateResponse("upload_form.html", {"request": request, "error": "Faltan archivos de Siprosa (se requieren 3)."}, status_code=400)
        ...
        if not file or not file.filename:
            return templates.TemplateResponse("upload_form.html", {"request": request, "error": "Debe cargar un archivo."}, status_code=400)
    ...
    # ­ƒæç NUEVO: log de creaci├│n de carga
    log_usage_event(
        user=user,
        action_type="file_upload",
...
# ======================================================================
@router.get("/otras-fuentes-externas", response_class=HTMLResponse)
...
async def crear_carga_otras_fuentes(
    ...
    # El registry.yml se encargar├í de despachar a la pampa u otros si matchea.
    # Si no matchea nada, caer├í en default o error, pero el flujo debe continuar.
    metadata = {
        "filename": final_filename,
...
@router.get("/cargas/historial", response_class=HTMLResponse)
def historial_cargas(
    ...
    try:
        # ­ƒæç ­ƒæç ­ƒæç AQU├ì USAMOS LA VERSI├ôN QUE DEJA VER TODO AL AUDITOR
    ...
    # ­ƒæç NUEVO: log de vista del historial
    log_usage_event(
        user=user,
        action_type="page_view",
...
def view_upload(
    ...
    # ­ƒæë si ya est├í finalizado, lanzamos aviso (idempotente)
    try:
    ...
    ctx = {
        ...
        "dup": request.query_params.get("dup") == "1",  # <-- ­ƒæê NUEVO
    }
    response = templates.TemplateResponse("upload_show.html", ctx)

    # ­ƒæç NUEVO: log de vista de detalle de proceso
    log_usage_event(
        user=user,
        action_type="page_view",
...
# ======================================================================
@router.get("/api/cargas/{upload_id}/status", response_class=JSONResponse)
def api_carga_status(
    ...
    """
    Devuelve info de la carga para la UI din├ímica (stepper, botones, etc.).
    ...
    # Para no-admin: alcanza con que el proceso est├® en estado ÔÇ£finalÔÇØ
    ...
    # ­ƒæë Notificaci├│n idempotente cuando entra a estado final
...
# ======================================================================
# API PARA ÔÇ£VISTAS GUARDADASÔÇØ
# ======================================================================
...
@router.get("/api/views", response_class=JSONResponse)
def api_views_list(
...
@router.get("/api/views/default", response_class=JSONResponse)
...
@router.get("/api/views/get", response_class=JSONResponse)
def api_views_get(
    view_id: str = Query("dashboard"),
...
@router.post("/api/views", response_class=JSONResponse)
...
@router.post("/api/views/default", response_class=JSONResponse)
...
@router.delete("/api/views/{view_pk}", response_class=JSONResponse)
...
def _norm_status(s: str) -> str:
    """
    Normaliza un estado posible (en espa├▒ol/variantes) a la clave est├índar.
    """
    s = (s or "").strip().lower()
    mapping = {
        "pendiente": "pending",
...
# ======================================================================
@router.post("/api/cargas/{upload_id}/avance", response_class=JSONResponse)
def api_carga_avance(
    ...
    Reglas:
    - Orden can├│nico: pending ÔåÆ classifying ÔåÆ processing ÔåÆ reviewing ÔåÆ dashboard ÔåÆ done
    - Al entrar a 'processing' y si falta normalized.xlsx ÔåÆ se dispara classify_and_process.
    - Para ir a 'dashboard'/'done' se exige normalized.xlsx (a menos que force=1).
    - Al llegar a estado finalizado/dashboard/tablero ÔåÆ dispara notificaci├│n ├║nica.
    """
    up = db_session.get(UploadModel, upload_id)
    if not up:
    ...
    if action == "next":
        if idx >= len(orden) - 1:
            return JSONResponse(
                {"ok": False, "msg": "Ya est├í en el ├║ltimo estado"},
                status_code=400,
            )
        ...
        if idx <= 0:
            return JSONResponse(
                {"ok": False, "msg": "Ya est├í en el primer estado"},
                status_code=400,
            )
        new_status = orden[idx - 1]
    elif action == "goto":
        ...
        if t not in orden:
            return JSONResponse(
                {"ok": False, "msg": "Destino inv├ílido"},
                status_code=400,
            )
        new_status = t
    else:
        return JSONResponse(
            {"ok": False, "msg": "Acci├│n inv├ílida"},
            status_code=400,
        )
            ...
            try:
                ...
                if _needs_processing(up):
                    return JSONResponse(
                        {
                            "ok": False,
                            "msg": "No se pudo preparar el normalized.xlsx autom├íticamente.",
                        },
                        status_code=409,
                    )
            except Exception as e:
    ...
    # si qued├│ finalizado ÔåÆ notificar
    ...
    # si entramos a 'processing' y hace falta procesar ÔåÆ disparar pipeline
    try:
...
@router.get("/api/descargar-final/{upload_id}/{filename}")
...
def api_tablero_ranking(
    ...
    line_alert_map = _detect_line_price_alerts(df).get("line_alerts", {})
    ...
    for rec in df.to_dict(orient="records"):
        ...
        # ­ƒæë NUEVO: Rubro (solo si no se sete├│ antes o est├í vac├¡o)
        ...
        rows_out.append(
            {
                ...
                "rubro": r.get("rubro", ""),  # ­ƒæê NUEVO
                "cantidad": float(
                    r["cantidad"] if r["cantidad"] > 0 else 1.0
                ),
                "positions": positions,
                "alerts": line_alert_map.get(str(r["nro"]), []),
            }
        )

    # ­ƒæç NUEVO: log de consulta de ranking
    log_usage_event(
        user=user,
...
# ======================================================================
# KPI Y GR├üFICOS DEL TABLERO
# ======================================================================
def compute_kpis_and_charts(df: pd.DataFrame):
    """
    Calcula KPIs y gr├íficos base del tablero.
    """
    kpis: Dict[str, Any] = {}
...
def compute_extra_charts(df: pd.DataFrame, col_map: dict):
    """
    Gr├íficos complementarios: top proveedores ajustado, donut posiciones, donut suizo.
    """
...
def _detect_line_price_alerts(
    ...
    item_col = _pick(df, ["Nro", "Numero", "Item", "Renglon", "N°", "Nº"])
    prov_col = _pick(df, ["Proveedor", "Oferente", "Empresa", "Supplier", "Provider"])
    price_col = _pick(df, ["Precio unitario", "Precio Unitario", "Precio", "PU", "Unit Price", "Price"])
    if not (item_col and prov_col and price_col):
        return result
    ...
    try:
        ...
        tmp = tmp[(tmp["_item_key"] != "") & (tmp["_price"] > 0)]
    except Exception:
        return result
...
def detectConsistencyAlerts(data: Dict[str, Any]) -> Dict[str, Any]:
    """
    Capa centralizada de inteligencia comercial para advertir inconsistencias.
    No corrige datos ni bloquea el tablero: solo contextualiza el analisis.
    """
    thresholds = dict(CONSISTENCY_THRESHOLDS)
    thresholds.update(data.get("thresholds") or {})
    alerts: List[Dict[str, Any]] = []
    kpi_badges: Dict[str, Dict[str, str]] = {}
    ...
    def add_alert(
        ...
        alerts.append(alert)
        badge_text = "Revisar dato" if severity == "critical" else "Dato atipico"
        for field in affected_fields:
            current = kpi_badges.get(field)
            if current and current.get("severity") == "critical":
                continue
            kpi_badges[field] = {
                "severity": severity,
                "label": badge_text,
    ...
    if presupuesto <= 0:
        add_alert(
            "info",
            "missing_data",
            "Presupuesto adjudicado no disponible",
            "No se detecto un presupuesto adjudicado comparable para contrastar los montos ofertados.",
            ["presupuesto_adjudicado"],
            {"presupuesto_adjudicado": presupuesto},
            "Revisar si el archivo original informa presupuesto, monto adjudicado o criterio de adjudicacion.",
        )
    ...
    if monto_adjudicado_suizo > 0 and monto_ofertado_suizo > 0 and monto_adjudicado_suizo > monto_ofertado_suizo:
        add_alert(
            ...
            "Monto adjudicado mayor al ofertado",
            (
                f"El monto adjudicado a Suizo ({_money_compact(monto_adjudicado_suizo)}) "
                f"supera el monto ofertado por Suizo ({_money_compact(monto_ofertado_suizo)})."
            ),
            ["monto_adjudicado_suizo", "monto_ofertado_suizo"],
            {
    ...
    if line_total:
        severity = "critical" if line_result["critical_count"] else "warning"
        add_alert(
            severity,
            "line_outlier",
            ...
            {
                "warning_count": int(line_result["warning_count"]),
    ...
    if counts["critical"]:
        quality = "Critico / revisar archivo original"
    elif counts["warning"]:
        quality = "Requiere revision"
    elif counts["info"]:
        quality = "Con observaciones menores"
    else:
        quality = "Sin alertas relevantes"
    ...
    return {
        ...
        "line_alerts": line_result["line_alerts"],
        "thresholds": thresholds,
    }


@router.get("/tablero/{upload_id}", response_class=HTMLResponse)
def tablero_show(
    request: Request,
    upload_id: int,
    user: User = Depends(
    ...
    # Ô£à NUEVO: URL para descargar el PDF del proceso
    ...
    # ­ƒæç NUEVO: log de vista del tablero
    log_usage_event(
        user=user,
        action_type="page_view",
        section="tablero",
...
def login_submit(
    ...
    # ­ƒæç NUEVO: log de inicio de sesi├│n
    log_usage_event(
        user=u,
        action_type="login",
        section="auth",
...
def logout(request: Request):
    # ­ƒæç NUEVO: log de logout si hay usuario
    u = get_current_user(request)
    if u:
        log_usage_event(
            user=u,
...
# ======================================================================
# MI CONTRASE├æA (USUARIO LOGUEADO)
# ======================================================================
...
# ======================================================================
# OLVID├ë MI CONTRASE├æA
# ======================================================================
@router.get("/password/olvido", response_class=HTMLResponse)
...
@router.get("/password/restablecer", response_class=HTMLResponse)
    ...
    if not email:
        return templates.TemplateResponse(
            "password_reset.html",
            {
                "request": request,
                "token": "",
                "error": "El enlace no es v├ílido o ya venci├│.",
            },
        )
...
# ======================================================================
@router.get("/descargas/{upload_id}")
...
@router.get("/reportes/proceso/{upload_id}")
def descargar_reporte_proceso(
    ...
    # 2) ­ƒöÆ Verificar visibilidad por grupos (auditor ve todo).
    #    Si el proceso es del propio usuario, SIEMPRE permitir.
    vis_ids = visible_user_ids_ext(db_session, user)
    if (up.user_id != user.id) and (up.user_id not in vis_ids):
        raise HTTPException(
    ...
    # 3) ­ƒöÆ Regla: solo ADMIN puede ver antes de finalizado
    role = (user.role or "").lower()
    is_admin = role == "admin"
    ...
    # 7) Posiciones (si est├ín serializadas en el modelo)
    posiciones = []
...
# === Informe PDF ============================================================
@router.get("/informes/{upload_id}/pdf", name="informe_pdf")
def informe_pdf(
    upload_id: int,
    request: Request,
    db: Session = Depends(get_db),
...
@router.get("/grupos/{group_id}/editar", response_class=HTMLResponse)
...
@router.post("/grupos/{group_id}/actualizar")
def grupos_update(
    request: Request,
    group_id: int,
    name: str = Form(...),
...
@router.post("/grupos/{group_id}/miembros/agregar")
...
@router.post("/grupos/{group_id}/miembros/{member_id}/rol")
...
@router.get("/grupos/{group_id}/miembros/{member_id}/eliminar")
...
@router.get("/grupos/{group_id}/eliminar")
def grupos_delete(
    request: Request,
    group_id: int,
    user: User = Depends(require_roles("admin", "supervisor")),
...
# ======================================================================
# PANEL DE PRESERVACI├ôN DE FILTROS / PRESETS
# ======================================================================
...
@router.get("/api/presets", response_class=JSONResponse)
...
@router.post("/api/presets", response_class=JSONResponse)
...
@router.delete("/api/presets/{pid}", response_class=JSONResponse)
...
@router.get("/api/presets/{pid}/apply")
